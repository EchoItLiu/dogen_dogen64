"""
Here, scipy.interpolate.interp1d is used.
Later, you can try other interpolation methods to compare their effects, such as:
① Linear interpolation method - Nearest Neighbour Interpolation

② Linear interpolation method - Bilinear interpolation

③ Linear interpolation method - Bicubic interpolation

④ Non-linear interpolation method (possibly applicable to this dataset) based on edge information interpolation:
i) ... ii)...
iii)...
iv)...
v)...
...
⑤ Nonlinear interpolation method (possibly applicable to this dataset) based on wavelet coefficients
And compare their advantages and disadvantages... Select the best one as the preferred interpolation method for other methods,
and analyze the reasons why it is good.
"""
...
# Interpolation of ts sequence features to make both
 ## left and right feet of equal length based
  ### on a dual-tower model
# It is recommended to use the default
 ## linear interpolation for now,
  ### as it is more stable~~~

# Then, perform the interpolation before
 ## normalization. Just write a separate
  ### method for this~~~

# Pay attention to the order of interpolation;
 ## it must be interpolation first,
  ### then normalization.

@@ Traverse through the 1d, 2d, 3d, and 4d interpolation and methods in scipy, as well as master_source@@, and:
 @ Time point interpolation @
  @ Spatial point interpolation @
  Common libraries for the methods used

$$ Address the LSTM token explosion issue
 and compare the differences between the
   DefaultCPUAllocator of the two,
     as well as the explosion points.
       Consider the solutions. Here, we
         will discuss some strategies and
           algorithms (of research value): $
$
$
$
$
...
class plan_model(nn.Module):
    ...
    def forward(self, original_signal, ts_features):
        ...
        params:
            original_signal: Tensor, shape [B, C, L] (C=2: l-r foot, L=Original length)
            ts_features: Tensor, shape [B, L, feature_dim]，即[B, L, C](The interpolated length is L.)
        ...
        if self.use_transformer:
            ...
            # lstm_out serves as the output
            lstm_out, (h_n, c_n) = self.lstm(ts_features)  # [B, num_cycles, hidden*2]
            # h_n serves as the output
            # _, (h_n, c_n) = self.lstm(ts_features)  # [D*layer_num, B, hidden*2]

            ts_features_global = lstm_out[:, -1, :]  # [B, hidden*2]
            #
            # ts_features_global = h_n[-1,:,:]
...
def load_dogen_data(dogen_path, train_ratio=0.5):
    ...
    for pkl_file in tqdm(pkl_files, desc="Load the pkl file..."):
        ...
        ts_data = ndstrarr2ndarray(ts_data).astype(np.float16)

        '''
        '''
        elapsed_time= ts_data[:,0]
        ts_data = ts_data[:,1:] # [num_cycles, 12]
...
def plot_training_results(train_losses, val_losses, train_accs, val_accs,
    ...
    plt.tight_layout()
    plt.savefig(r'D:\gait-in-neurodegenerative-disease-database-1.0.0\gait-in-neurodegenerative-disease-database-1.0.0\log_boards_model_pred_results\interp_ab_dogen_results.png', dpi=300, bbox_inches='tight')
    # plt.show()
    plt.cla()
    print("The visualization results have been saved to: interp_ab_dogen_results.png")
    plt.close()
...
def main():
    ...
    model_kwargs = {
        ...
        'ts_feature_dim': 12,
        # 'ts_hidden_dim': 128,
        # 'ts_hidden_dim': 16, # 16*2(D)
        'ts_hidden_dim': 4, # 4*2(D)
        # 'ts_num_layers': 2, # 2*2(D)
        'use_transformer': False,  # True: Transformer, False: LSTM
        'num_classes': 4,
        'fusion_hidden_dim': 256,
