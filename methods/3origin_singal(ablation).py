"""
# Only using the original signal
## Actually, by removing one of the towers inside the  DualTower...
### SingleTower structure(ablation)
"""
...
class plan_model(nn.Module):
    """
    ablate the ts-tower and fuse it into a single-tower structure
    """
    ...
    def __init__(self,
        ...
        self.out_layer = nn.Sequential(
            nn.Linear(self.original_output_dim, original_out_hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(original_out_hidden_dim, original_out_hidden_dim // 2),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(original_out_hidden_dim // 2, num_classes)
        )
        #
        #
        #
    ...
    def forward(self, original_signal):
        ...
        logits = self.out_layer(original_features)
...
def train_model(model, train_loader, val_loader, epochs=50, lr=1e-3,
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (original_signal, _, labels) in enumerate(pbar):
            original_signal = original_signal.to(device)
            labels = labels.to(device)

            logits = model(original_signal)
        ...
        with torch.no_grad():
            for original_signal, _, labels in val_loader:
                original_signal = original_signal.to(device)
                labels = labels.to(device)
...
def cross_validation_loo(model_class, original_signals, ts_features, labels,
    ...
    for train_idx, val_idx in logo.split(original_signals, labels, groups=groups):
        ...
        with torch.no_grad():
            ...
            logits = model(val_original_tensor)
...
def test_model(model, test_loader, device='cpu'):
    ...
    '''
    with torch.no_grad():
        for original_signal, _, labels in test_loader:
            original_signal = original_signal.to(device)
            labels = labels.to(device)
...
def plot_training_results(train_losses, val_losses, train_accs, val_accs,
    ...
    plt.tight_layout()
    plt.savefig(r'D:\gait-in-neurodegenerative-disease-database-1.0.0\gait-in-neurodegenerative-disease-database-1.0.0\log_boards_model_pred_results\singleTower_results.png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.cla()
    print("The visualization results have been saved to: singleTower_results.png")
    plt.close()
