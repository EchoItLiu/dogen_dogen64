'''
Then, we look forward to a new cross-entropy
...
def calculate_entropy_importance(input_data, top_k_entropy_th, is_ts):
    """
    Calculate information entropy importance for feature selection.

    Args:
        left_right_data (np.ndarray): Input feature map [C, L]/[L,C] where C is channel,
    ...
    # print ('d1_shape:', input_data.shape)
    ...
    try:
        ...
        if input_data.ndim!=2:
            raise ValueError(f'The input must be a 2D array [C, L]. Current dimension:{input_data.ndim}')

        # Softmax calculation along C dimension (axis=0)
        # For numerical stability, subtract max value before exponentiation
        # print ('d3_shape:', input_data.shape)
        max_vals = np.max(input_data, axis=0, keepdims=True)
        # print ('---1---')
        ...
        if entropy_max - entropy_min > 1e-8:
            entropy_normalized = (entropy - entropy-entropy_min) / (entropy_max - entropy_min)
        ...
        input_data_k = input_data[:, sorted_entropy_normalized_idx]
        ...
        if hasattr(input_data, 'shape'):
            return np.zeros((input_data.shape[0], min(top_k_entropy_th, 110)))
        else:
...
def load_dogen_data(dogen_path, train_ratio=0.5, top_k_entropy_th = 200):
    ...
    for pkl_file in tqdm(pkl_files, desc="Load the pkl file..."):
        ...
        print ("calculate information entropy for every signal... ")

        # left_right_data_ied = calculate_entropy_importance(left_right_data, top_k_entropy_th, is_ts=False)

        # left_right_data_ied_length = left_right_data_ied.shape[1]
        ...
        '''
        # Here, the original signals of the
         ## left and right feet are subjected to
          ### information entropy extraction,
          #### instead, using the ts temporal feature.
        '''
        # [C,k] → [k,12]
        # print ('ts_data_shape:', ts_data.shape)
        ts_data_ied = calculate_entropy_importance(ts_data, top_k_entropy_th, is_ts=True)
        # print ('singal_info_Enk_shape_before:', ts_data_ied.shape)
        ts_data_ide = np.transpose(ts_data_ied,(1,0))
        # print ('singal_info_Enk_shape:', ts_data_ide.shape)


        #
        ts_features.append(ts_data_ide)
        basenames_l.append(base_name.lower())


    lr_data_features = np.array(lr_data_features)  # [N, 2, L]
    ts_features = np.array(ts_features)           # [N, num_cycles(k), 12]
...
def plot_training_results(train_losses, val_losses, train_accs, val_accs,
    ...
    plt.tight_layout()
    plt.savefig(r'D:\gait-in-neurodegenerative-disease-database-1.0.0\gait-in-neurodegenerative-disease-database-1.0.0\log_boards_model_pred_results\enK_ab_dogen_results.png', dpi=300, bbox_inches='tight')
    # plt.show()
    plt.cla()
    plt.close()
    #
    print("The visualization results have been saved to: enK_ab_dogen_results.png")
    ...
    if not os.path.exists(dogen_path):
        ...
        (train_original_signals, train_ts_features, train_labels,
         test_original_signals, test_ts_features, test_labels) = load_dogen_data(
            dogen_path, train_ratio=0.5, top_k_entropy_th = topKentropy_th
        )



    # ==================== Step 2. Encapsulated dataset ====================
